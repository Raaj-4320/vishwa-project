  ...
  return (
    <AuthLayout title={roleCopy.title} subtitle={roleCopy.subtitle}>
      ...
      <div className="auth-links">
        ...
        <Link to={registerPath}>{roleIntent === 'admin' ? 'Register admin account request' : 'Create account'}</Link>
      </div>
    </AuthLayout>
  );
};
...
export const RegisterPage = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      navigate(isAdminRegister ? '/login?role=admin' : '/customer', { replace: true });
    } catch {
  ...
  return (
    <AuthLayout title={isAdminRegister ? 'Register admin access request' : 'Create account'} subtitle={isAdminRegister ? 'Create an account and sign in to request admin privileges.' : 'Register a secure customer account to get started.'}>
      <form className="auth-form" onSubmit={onSubmit}>
        ...
        {error ? <p className="error-text">{error}</p> : null}
        <button className="btn" type="submit" disabled={loading}>{loading ? 'Creating account…' : isAdminRegister ? 'Register admin request' : 'Register as customer'}</button>
      </form>
      <div className="auth-links">
