...
const medicineBodyPartial = medicineBodyBase.partial().superRefine((val, ctx) => {
  ...
  if (val.totalPurchase !== undefined && val.totalSold !== undefined && val.totalSold > val.totalPurchase) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['totalSold'], message: 'totalSold cannot exceed totalPurchase' });
  }
});
